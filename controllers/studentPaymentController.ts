...
import StudentPaymentModel, { StudentPaymentInterface } from '../models/studentPaymentModel';

const getAllPayments = async (req: Request, res: Response) => {
  try {
    const studentPayments = await StudentPaymentModel.find();
    res.status(200).json({
      success: true,
...
const updatePayment = async (req: Request, res: Response) => {
  try {
    const updatedStudentPayment = await StudentPaymentModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
const deletePayment = async (req: Request, res: Response) => {
  try {
    const deletedStudentPayment = await StudentPaymentModel.findByIdAndDelete(req.params.id);
...
export { getAllPayments, createPayment, getPaymentById, updatePayment, deletePayment };
